...
for i = 1:length(U)
	if ~isempty(U{i})
		siz(i) = size(U{i},1);
		H = Global.GL_HOSVD.gl_hosvd.ndim_unfold(T, i);           % puts arrays of data along specified dimension i
		T = Global.GL_HOSVD.gl_hosvd.ndim_fold(U{i}*H, i, siz);   % allows U values of filtered svd to be projected onto 
