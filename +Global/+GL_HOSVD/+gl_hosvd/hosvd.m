...
for i = 1:P       % for all b values
	if dim(i)
		A = Global.GL_HOSVD.gl_hosvd.ndim_unfold(T, i);    % reshapes data into 2D, lays out every slice for every b value
		% SVD based reduction of the current dimension (i)
		[Ui svi toli] = Global.GL_HOSVD.gl_hosvd.svdtrunc(A, tol(i));
		U{i} = Ui;
		UT{i} = Ui';
		sv{i} = svi;
		tol(i) = toli;
...
S = Global.GL_HOSVD.gl_hosvd.tprod(T, UT);
