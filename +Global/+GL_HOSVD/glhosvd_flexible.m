...
[Sigma0, U0]   =   Global.GL_HOSVD.gl_hosvd.hosvd(full(nim));
th            =   kglobal*nsig*sqrt( 2*log(length(nim(:))) );
Sigma0(find(abs(Sigma0(:))<th))=0;
pre_im        =   Global.GL_HOSVD.gl_hosvd.tprod(Sigma0, U0); %%%%the prefiltered data
...
function  [X W]   =   Low_rank_SSC( Y1,Y2, nsig)
  ...
  [Sigma2 U1] = Global.GL_HOSVD.gl_hosvd.hosvd2(full(Y1),full(Y2));
  
  Sigma2(abs(Sigma2) < nsig*sqrt( 2*log(length(Y1(:))) ) )=0;
  r   =   sum( abs(Sigma2(:))>0 );
  X   =   Global.GL_HOSVD.gl_hosvd.tprod(Sigma2, U1); 
  wei =   1/(1+r);
  W   =   wei*ones( size(X) );
  X   =   X*wei;
return;
