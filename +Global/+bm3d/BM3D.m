...
if isa(profile, 'string') || isa(profile, 'char')
    profile = Global.bm3d.BM3DProfile(profile);
end
...
if bitand(stage_arg, Global.bm3d.BM3DProfile.HARD_THRESHOLDING)
    
    [t_forward, t_inverse, hadper_trans_single_den, ...
        inverse_hadper_trans_single_den, Wwin2D] = get_transforms(profile, true);

    [y_hat, ht_bm] = Global.bm3d.bm3d_thr_colored_noise(z, hadper_trans_single_den, ...
    ...
    if (profile.denoise_residual)
       ...
       remains_PSD = process_psd_for_nf(remains_PSD, psd_k, profile);
        ...
        if(min(max(max(remains_PSD, [], 1), [], 2)) > 1e-5)
            % Re-filter
            [y_hat, ht_bm] = Global.bm3d.bm3d_thr_colored_noise(double(y_hat + remains), ...
...
if bitand(stage_arg, Global.bm3d.BM3DProfile.WIENER_FILTERING)
    ...
    [y_est, wie_bm] = Global.bm3d.bm3d_wiener_colored_noise(z, single(y_hat), hadper_trans_single_den, ...
    ...
    if (profile.denoise_residual)
        ...
        if(min(max(max(remains_PSD, [], 1), [], 2)) > 1e-5) 
            ...
            % Re-filter
            [y_est, wie_bm] = Global.bm3d.bm3d_wiener_colored_noise(double(y_est + remains), y_est, ...
