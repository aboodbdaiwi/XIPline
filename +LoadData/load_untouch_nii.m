...
function nii = load_untouch_nii(filename, img_idx, dim5_idx, dim6_idx, dim7_idx, ...
   ...
   %
   [nii.hdr,nii.filetype,nii.fileprefix,nii.machine] = LoadData.load_nii_hdr(filename);
   if nii.filetype == 0
      nii.hdr = LoadData.load_untouch0_nii_hdr(nii.fileprefix,nii.machine);
      nii.ext = [];
   else
      nii.hdr = LoadData.load_untouch_nii_hdr(nii.fileprefix,nii.machine,nii.filetype);
      %nii.hdr = LoadData.load_untouch_header_only(nii.fileprefix,nii.machine,nii.filetype);
      %  Read the header extension
      %
      nii.ext = LoadData.load_nii_ext(filename);
   end
   %  Read the dataset body
   %
   [nii.img,nii.hdr] = LoadData.load_untouch_nii_img(nii.hdr,nii.filetype,nii.fileprefix, ...
