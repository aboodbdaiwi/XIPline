...
tmp = wf.k(1,:,1:2);
tmp = reshape(tmp,sum(wf.ind(1,:)),nbvalues,size(d,1)/nbvalues,2);
...
switch dim
    case 2
        ...
        set(gcf,'name',figstr);
        if ~isempty(fname)
            print([fname '_diffusion'],'-dpng','-r600');
        end
    case 3
        for lb = 1:nbvalues
...
if ~isempty(fname)
    ...
    write_dicom_cchmc(fname,squeeze(bbabs),h,inp);
end
