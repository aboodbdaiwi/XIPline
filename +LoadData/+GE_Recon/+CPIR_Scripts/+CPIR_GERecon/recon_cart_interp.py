...
# -*- coding: utf-8 -*-
"""
Cartesian fidall reconstruction interpolated
Created: 3/2026
@author: Rolf Schulte
"""
...
if len(sys.argv)>6:
  ...
  fnlog = fidalldir + 'log/' + now.strftime("%Y%m%d_%H%M%S") + '_recon_cart.txt'
...
cmd = ['recon_cart',datadir+rawdataname,'[]',wfn,zf,delay,lb,datadir+rawdataname,comb_time,grdwrp,'0']
if dofifo:
  sor.run_fifo(cmd)
else:
  sor.run(cmd)
