...
# -*- coding: utf-8 -*-
"""
2D/3D gridding reconstruction (+ stack-of-2D) interpolated
3/2026
...
if len(sys.argv)>6:
  ...
  fnlog = fidalldir + 'log/' + now.strftime("%Y%m%d_%H%M%S") + '_recon_grid_interp.txt'
...
cmd = ['recon_grid_interp',rawin,'[]',wfn,mtx,interp_factor,delay,lbt,datadir+rawdataname,mask_fac,coco,plt,grdwrp]
if not use_vre:
  # on host
  if dofifo:
    sor.run_fifo(cmd)
...
# finishing up
print('recon_grid_interp.py done')
if len(sys.argv)>6:
  sys.stdout.close()
