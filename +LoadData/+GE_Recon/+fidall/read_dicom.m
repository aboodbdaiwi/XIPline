...
fns = dir(fname);                 % list all files in directory
ii_dcm = [];                      % index for including files
ii_sort = [];                     % index for sorting files
for l=1:length(fns)
    if ~fns(l).isdir              % exclude directory names
        fn = [fns(l).folder filesep fns(l).name];
        if isdicom(fn)            % check if file names are dicoms
...
fn = [fns(1).folder filesep fns(1).name];
if verb, fprintf('fns(1).folder/name = %s\n',fn); end
tmp = dicomread(fn);                   % read single file -> get data size
data = single(zeros(size(tmp,1),size(tmp,2),nf));  % preallocate as single
data(:,:,1) = tmp;
for l=2:nf
    fn = [fns(l).folder filesep fns(l).name];
    if verb, fprintf('fns(%g).folder/name = %s\n',l,fn); end
    data(:,:,l) = dicomread(fn);
end
...
for l=1:nn
    ...
    header(l) = dicominfo(fn,'dictionary',dict); 
end
