...
if max(fov)>fov(1)
    warning('max(fov)(=%g)>fov(1)(=%g)',max(fov),fov(1));
end
mtx = mtx(:).';
...
if scheme~=3 && isscalar(samp), samp = [samp 1]; end
...
if ~exist('ncok','var'),     ncok = []; end
if ~exist('do_vap','var'),   do_vap = []; end
if isempty(do_vap),          do_vap = false; end
...
switch scheme    
    case {1,2}     % uniform sampling: 1=rectangular, 2=circular
        ...
        if scheme==2
            ...
            ii = rad<=0.51;                 % index inside circle
            k = k(:,ii,:);
            grd_drt = 1;
        else
            grd_drt = sqrt(dim);
...
nexc = size(k,2);
if isodd(nexc)
    fprintf('nexc(=%d) is odd; removing one sample: max(sum(k.^2,3))\n',nexc);
    [~,ii] = max(sum(k.^2,3));
    k = k(:,[1:(ii-1),(ii+1):end],:);
    dcf = dcf(:,[1:(ii-1),(ii+1):end]);
    apo = apo(:,[1:(ii-1),(ii+1):end],:);
    nexc = nexc-1;
    if (size(k,2)~=nexc) || (size(apo,2)~=nexc) || (size(dcf,2)~=nexc)
        error('size(k/apo/dcf,2)(=%d %d %d) ~= nexc(=%d)',...
            size(k,2),size(apo,2),size(dcf,2),nexc);
    end
end
...
switch ktraj
    ...
    case 2         % randomise
        ...
        k = k(:,ii,:);
        dcf = dcf(:,ii,:);
    case 3         % centre to ouside
        [~,ii] = sort(sqrt(sum(k.^2,3)));
        k = k(:,ii,:);
        dcf = dcf(:,ii,:);
    case 4         % outside to centre
        [~,ii] = sort(sqrt(sum(k.^2,3)),'descend');
        k = k(:,ii,:);
        dcf = dcf(:,ii,:);
    otherwise
        warning('ktraj(=%g) unknown',ktraj);
end
...
ga_phas = pi/min(res)/abs(gyrogamma(nucleus));
...
switch crush(1)
    case 0    % no crusher
        ng_crush = 0;
        ng_acq = 0;
    case 1    % same dir as phase encode;crush(2)=factor kmax
        if length(crush)<2, crush(2) = 1.5; end
        [G_crush,ng_crush] = gradient_lobe(crush(2)*ga_phas,0,gmax/grd_drt,...
            smax/grd_drt,gdt,true);

        ng_acq = ceil(npts/bw/gdt);
    case 2    % refocusing (balanced)
        G_crush = -G_phas;
        ng_crush = ng_phas;
        ng_acq = ceil(npts/bw/gdt);
    otherwise
...
if do_vap
    grad = kmax*repmat(gg,[1 1 dim]);
    if crush(1)>0
        switch crush(1)
            case 1
                crush_scale = 1;        % crusher scaled with agX
                warning('crusher scaled with vap_ag; use do_vap=false for max crush');
            case 2, crush_scale = 2*kmax;
        end
        ii = (1:ng_crush)+ng_tot-ng_crush;
        grad(ii,:,:) = repmat(crush_scale*G_crush.',[1 1 dim]);
    end
else
    grad = bsxfun(@times,gg,ksca);
    switch crush(1)
        case 1     % same dir as phase encode;crush(2)=factor kmax
            % crusher always same max area
...
ind1 = [];
if any(ncok>0)
    if length(ncok)<2, ncok(2) = 0; end
    if any(abs(ncok-floor(ncok)) > 1d-10)
        error('ncok(=%g %g) not a natural number',ncok(1),ncok(2));
    end
    if ncok(1)==1, error('ncok(1)(=%g)==1',ncok(1)); end
    ind1 = true(2*nexc,1);
    ind1(1:ncok(1):end,1) = false;
    ind1 = [false(ncok(2),1) ; ind1];  % add dummy scans to beginning
    ni = find(cumsum(ind1)==nexc);
    nexc = ni(end);
    ind1 = ind1(1:nexc,1);
    if do_vap
        ksca_tmp = zeros(1,nexc,dim);
        ksca_tmp(:,ind1,:) = ksca;
        ksca = ksca_tmp;
    else
        grad_tmp = zeros(size(grad,1),nexc,dim);
        grad_tmp(:,ind1,:) = grad;
        grad = grad_tmp;
    end
    fprintf('Adding interleaved spectra: ncok=[%d %d]; nexc=%d+%d=%d\n',...
        ncok(1),ncok(2),sum(ind1),sum(~ind1),length(ind1));
end
...
if verb>0
    ...
    switch dim
        ...
        case 2
            ...
            axis square
            axis(1.05*max(abs(ksca(:)))*[-1 1 -1 1]); grid on
        case 3
            subplot(2,2,1);
...
if verb>1
    ...
    wf.nviews = length(ind1); wf.inds = ind1;
    wf.scheme = scheme;
...
if (islogical(fname) || isnumeric(fname))
    if fname
        ...
        ktraj_str = '';
        if ktraj~=1
            ktraj_str = ['_' num2str(ktraj)];
            ktraj_str = strrep(ktraj_str,'-','m');
        end
        fname = sprintf('mrsi%gD_%s_scheme%g%s_fov%s_mtx%s_bw%g_npts%g_nexc%g_gmax%g_smax%g',...
            dim,nucleus,scheme,ktraj_str,fov_str,mtx_str,bw,npts,...
            nexc,round(gmax_act*1d3),round(smax_act));
        if any(ncok>0), fname = sprintf('%s_ncok%d_%d',fname,ncok(1),ncok(2)); end
        if crush>0, fname = sprintf('%s_crush%d_%d',fname,crush(1),crush(2)); end
...
if ~isempty(fname)
    ...
    if do_vap
        xyz = {'x','y','z'};
        for ll=1:dim
            write_fdl([fname '_ag' xyz{ll} '.fdl'],ksca(1,:,ll)/kmax,'agrad');
        end
    end
    save(fname,'k','dcf','t','out',...
        'fov','mtx','scheme','bw','npts','gmax','smax','fname',...
         ...
         'gmax_act','smax_act','apo','apotune','ncok','ind1');
else
    out = [];
end
