...
%  lbt   Line-broadening in time domain [Gaussian, exp.] [Hz]       ([0 0])
...
if ~isempty(t) && any(abs(lbt)>1d-10)
    nrep = npts/size(t,1);
    ...
    fil = exp(-(pi*lbt(1)*tt).^2/(4*log(2))); % RFS 10/2020: 2*log(2)->4*log(2)
    if abs(lbt(2))>1d-10
        fil = fil.*exp(-pi*lbt(2)*tt);
    end
    if plt
