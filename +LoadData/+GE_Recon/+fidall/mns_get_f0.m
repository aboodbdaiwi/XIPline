...
if ~isnumeric(d) || isempty(d)
    ...
    [d,h] = read_p(d);       % uses single for d>1GB
end
...
f13Cpyr = mns_freq_converter(f0,'13C','pyruvate',false);
...
if plt
    ...
    text(-bw/40,1.01,sprintf('13C Pyr f0=%d[Hz]',round(f13Cpyr)),'FontSize',12);
    text(-bw/40,0.94,sprintf('129Xe lung f0=%d[Hz]',round(f129Xe)),'FontSize',12);
    
    
    if ~isempty(fname)
        if isdeployed
            print([fname '.png'],'-dpng','-r300','-painters');
        else
