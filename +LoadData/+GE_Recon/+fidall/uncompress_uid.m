...
for l=1:(2*length(short_uid))
    ...
    if sval1>255
        translation_used = translation_used + 1;
        if verb>2 
            fprintf('l=%d %d (>255)\n',l,sval1);
        end
...
if ~isempty(nwarn)
    fprintf('sval1 = %s\n',num2str(nwarn));
    warning('sval1 not in correction list; setting to 17 (->''00'')');
end
if translation_used>0
    fprintf('Attention: sval1>255 (%dx); switch character set before reading header via\n',...
        translation_used);
