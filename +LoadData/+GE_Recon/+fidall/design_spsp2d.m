...
if abs(dt_spec/hwdt-round(dt_spec/hwdt)) > 1d-12
    ...
    warning('dt_spec (=%g[us]) not multiple of hwdt (=%g[us]); rounding dt_spec to %g[us]',...
        dt_spec*1d6,hwdt*1d6,tmp*1d6);
end
...
fprintf('fname=%s\n',fname);
if sv
    fileid = fopen([fname '_design_spsp2d_output.txt'],'w+t');
else
    fileid = 1;
end
...
[ln1,ln2] = find(b2d,1);
plot(F(1,:)*bw(1)*1d-3,[0 0 1 1 0 0],'b',...
    ax{1}(mask(:,ln2))*bw(1)*1d-3,b2d(mask(:,ln2),ln2),'c.');
axis([-bw(1)/2d3 bw(1)/2d3 -0.1 1.15]); grid on; title('spatial');
xlabel('[mm]');
...
gg = [g*1d-3 ; ones(size(g))*2*pi/gyrogamma(nucleus)];
...
plot(bw(1)*xx*1d-3,real(A1*rfX),'r');
hold off;
...
if do_blosi
    ...
    [Mxy,Mz,ax]=blosi(rf,gg,[inp.thick*4d3 bw(2)*2],mtx_blosi,[],nucleus);
    xlabel('freq [Hz]'); ylabel('z [mm]');
    drawnow
end
...
if sv
    ...
    figure(11); print('-dpng','-r300',[fname '_fig11']);
    figure(12); print('-dpng','-r300',[fname '_fig12']);
    figure(13); print('-dpng','-r300',[fname '_fig13']);
    if do_blosi, figure(14); print('-dpng','-r300',[fname '_fig14']); end
...
function [g,ind_rf,nwf,nplat,nramp] = sub_design_grad(nspec,dt_spec,hwdt,flyback,...
    gmax,smax,gmax_fac,rampsamp,add_refo)
...
if ~flyback   % bi-directional trajectory    
    ...
    if add_refo
        g = [g 0.5*grew];
    %else
    %    ind_rf = ind_rf(1,1:end-length(grew));
    end
...
switch what
    ...
    case 5    % alias outside; CS shifted by half; large BW (no Lac+PyrH)
        ...
        par.halfshift = true;
        par.flyback = false;
    case 6    % nowafa7 - 1H flyback pulse for 2D MRSI: excite metabolites, not H2O+fat
        ...
        par.reg = 1d3;
        par.rampsamp = true;
    case 7    % nowafa40 - 1H flyback pulse for 3D MRSI: excite metabolites, not H2O+fat
        % stop-freq: H2O=4.7-0.1ppm; lipid=1.3+0.1ppm
        % pass-freq: 4.1 & 1.9ppm
        % set spsp_fatx1=-235
        par.nucleus = '1H';
    ...
    case 9    % 1H flyback pulse for 2D MRI: excite H2O, no fat
        ...
        par.do_real = true;
        par.rampsamp = 1;% 2;
    case 76    % 7T nowafa7 - 1H flyback pulse for 2D MRSI: excite metabolites, not H2O+fat
        % stop-freq: H2O=4.7-0.1ppm; lipid=1.3+0.1ppm
        % pass-freq: 4.1 & 1.9ppm
        % set spsp_fatx1=-550
        par.nucleus = '1H';
