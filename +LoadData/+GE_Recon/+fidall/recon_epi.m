function [bb,bbabs] = recon_epi(d,h,wfn,mtx,delay,fname,ref_corr,verb,coco)
...
%      coco   Coil combination via mri_coil_combine.m          ([1 0 0 16])
%             [method nper save_nococo mtx_coco]
%
%        bb   Reconstructed data (mtx,mtx,#exc,#metabs,#slices,(#coils))
%     bbabs   Magnitude/real data (always coil combined)
%
% 10/2024  Rolf Schulte
if (nargin<1), help(mfilename); return; end

thresh = 0.3;                % threshold for phase from correlation
...
if isfield(wf,'flyback')
    flyback = wf.flyback;
else
    warning('wf.flyback field not found; setting to false');
    flyback = false;
end
...
%% automatic delay determination
if ref_corr(1)==5
    if delay~=0, warning('ref_corr(1)(=5) and delay(=%g)~=0',delay); end
    frac = 0.1;
    delays = (-10:0.1:10);
    ff = zeros(1,length(delays));
    for ll=1:length(delays)
        [~,btmp] = recon_epi(d,h,wf,[],delays(ll),'',0,0,false);
        n2 = size(btmp,2);
        ii = false(1,n2); 
        ii(1,1:round(n2*frac)) = true;
        ii(1,round(n2*(1-frac)):n2) = true;
        btmp = btmp(:,ii,:,:,:);
        ff(1,ll) = sum(btmp(:).^2);
    end
    [~,ind] = min(ff);
    delay = delays(1,ind);
    fprintf('delay calibration: delay=%g [us]\n',delay);
    clear btmp
end


%% matrix size
mtx_acq = wf.mtx;                      % nominal matrix size of acquisition
...
[nexc,~,n3,n4,nslices,ncoils] = size(d);
...
%% adjusting delay for
if ~wf.rampsamp && ~flyback && (ref_corr(1)==0)
    dt = 1d6/bw;
    if verb>0
        fprintf('Subtracting dt/2=1/bw/2=%g [us] from delay = %g [us]\n',...
            dt,delay-dt/2);
    end
    delay = delay-dt/2;
end
...
if ((abs(xloc)>0.01) || (abs(yloc)>0.01))
    shft = mtx_acq.*[xloc yloc]*1d-3./fov;
else
    shft = [];
end
...
if do_ref
    ...
    if ref_corr(1)==2, wf.ref(4)=0; end
    if wf.ref(4)>0
...
if do_ref
    ...
    dtmp = raw2grid(d,ischop(h),wf.k,shft,[],[],wf.ind,delay*1d-6,bw,...
        false,[],verb>0);
    ...
    dtmp_ref = raw2grid(d(ind_ref1,:,:,:,:,:),ischop(h),wf.k,shft_ref,...
...
if wf.rampsamp
    if verb>0, fprintf('\tramp sampling -> 1D gridding\n'); end
    ...
    if verb>0, fprintf('\tplateau sampling -> 1D fft\n'); end
    dd = dd(1:mtx_acq(1),:,:,:,:);     % fft skips symmetric point
    % dd = dd(2:end,:,:,:,:);          % fft skips symmetric point
...
if do_ref
    ...
    switch ref_corr(1)
        ...
        otherwise, warning('ref_corr(1)(=%g) unknown; skipping',ref_corr(1));
    end
end
...
if mtx(2)~=mtx_acq(2)                       % Fourier interpolation
    bd = truma(bd,false,[size(bd,1) mtx(2) nslices nrep ncoils]);
end
...
if ncoils>1
    if (coco(3)>0) && ~isempty(fname)
        ...
        save([fname '_nococo.mat'],'-mat','-v7.3',...
            'bb','h','wf','mtx','mtx_acq','delay','fname',...
            'ref_corr','verb','shft','nintl');
    end
    if coco(1)>-1
        mtx_coco = [];
        if coco(4)>0, mtx_coco = coco(4); end
        bb = mri_coil_combine(bb,coco(2),'',coco(1),mtx_coco,[],true,2);
    else
...
if ~isempty(fname)
    ...
    % bbabs = single(bbabs);   % convert double (64bit) to single (32bit) precision
...
if verb>1
    ...
    imagesc_row(bbplt,[],'inm',false,true);
    colormap(gray);
...
ttv = toc(timerVal);
fprintf('recon_epi: runtime = %g [m] %02g [s]\n',...
...
if false
    ...
    tmp = fft(fftshift(bsxfun(@times,ifftshift(ifft(bdref(:,2:2:end,:,:,:),...
        zf,2),2),phafu2),2),[],2);
