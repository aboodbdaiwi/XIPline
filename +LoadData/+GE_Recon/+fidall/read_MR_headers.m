...
if nargin==1
    headerID = 'all';
    datatype = 'raw';
    fprintf('Datatype not specified assuming that the data is a "raw file" \n');
elseif nargin == 2
    datatype = 'raw';
    fprintf('Datatype not specified assuming that the data is a "raw file" \n');
end
...
switch headerID
    case 'all'
        ...
        for ii = 1:length(header_list)
            fseek( fid, offset, 'bof');
            switch header_list(ii)
                case 1 % rdb_hdr
            ...
            offset = offset + header_lengths(header_list(ii));
        end
    ...
    case 'qimg'
        ...
        for ii = 1:length(header_list)
            if header_list(ii)==12		% PIXEL header
                fseek( fid, offset, 'bof');
...
elseif strcmp(datatype,'raw')
    hdr_sizes_file = {'/export/home/signa/tools/hdr_sizes.txt', fullfile(fileparts(mfilename('fullpath')), 'hdr_sizes.txt')};
    % +++ rfs: isfile function not existing on Matlab 8.1
    % hdr_sizes_file = hdr_sizes_file(isfile(hdr_sizes_file));
    ii = false(1,length(hdr_sizes_file));
    for l=1:length(hdr_sizes_file)
        if exist(hdr_sizes_file{l},'file')
            ii(1,l) = true;
            break;
        end
    end
    hdr_sizes_file = hdr_sizes_file(ii);
    % --- rfs
    ...
    if ~isempty(hdr_sizes_file)
        hdr_sizes_file = hdr_sizes_file{1};
        fid = fopen(filename,'r','l');
        rdbm_rev = fread(fid, 1, 'float32');
        rdbm_rev = round(rdbm_rev*1000)/1000;
        fclose(fid);

        % +++ rfs: bug fix for non-existant 14.4 -> 14.3
        fprintf('RFS: header release = %g\n',rdbm_rev);
        if floor(rdbm_rev) == 14
            if rdbm_rev >  14.3
                fprintf('RFS: rdbm_rev > 14.3 -> setting to 14.3\n');
                rdbm_rev = 14.300;
            end
        end
        % --- rfs ---

        hdr_szs = read_headerinfo(hdr_sizes_file, rdbm_rev);
        if hdr_szs(1) ~= -1
