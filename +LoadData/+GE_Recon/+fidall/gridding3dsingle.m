...
%  compile via:  mex -compatibleArrayDims gr3dKB_float_mex.c
%     =INPUT=
...
%
%  8/2024 RFS bug fixes + converted to single
% ========================================================================
...
% RFS: adding checks
if any(isnan(kvec(:))),  error('kvec contains NaN'); end
if any(isinf(kvec(:))),  error('kvec contains inf'); end
if any(isnan(kdata(:))), error('kdata contains NaN'); end
if any(isinf(kdata(:))), error('kdata contains inf'); end
if any(isnan(dcf(:))),   error('dcf contains NaN'); end
if any(isinf(dcf(:))),   error('dcf contains inf'); end
if size(dcf,1)~=size(kdata,2)
    error('size(dcf,1)(=%d)~=size(kdata,2)(=%d)',size(dcf,1),size(kdata,2));
end
if size(kvec,1)~=size(kdata,2)
    error('size(kvec,1)(=%d)~=size(kdata,2)(=%d)',size(kvec,1),size(kdata,2));
end
timerVal = tic;                   % RFS: record execution time
...
if changeN && padN(1)>=imgN(1) && padN(2)>=imgN(2) && padN(3)>=imgN(3)
    ...
    x0 = round( (grsizePad-grsize)/2 );     % RFS
...
if crop
    % x0  = floor( (a-1)*imgN/2 );
    x0  = round( (a-1)*imgN/2 );       % RFS
    img = img( x0(1)+(1:imgN(1)), x0(2)+(1:imgN(2)), x0(3)+(1:imgN(3)), :);
end


%% info
fprintf('gridding3dsingle: runtime = %.2f [s]\n',toc(timerVal));


end      % gridding3dsingle.m
