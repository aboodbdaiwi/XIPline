function [out_data] = raw2grid(data,chp,k,shft,bw_filt,dec,ind,delay,...
    bw_acq,plt,ind_pc,verb)
%out_data = raw2grid(data,chp,k,shft,bw_filt,dec,ind,delay,bw_acq,plt,...
%                    ind_pc,verb)
...
%           dimension (1,kspace)
%    delay  Gradient delay time [s] (somewhere around -10d-6 to 10d-6)  [s]
...
if ~isempty(delay)
    if abs(delay)>1d-9
        ...
        if bw_acq<1000, warning('bw_acq(=%g) < 1000 [Hz]',bw_acq); end
        frac = abs(delay*bw_acq);
        cfrac = ceil(frac);
        % fprintf('frac=%g; cfrac=%g; frac-cfrac+1=%g\n',frac,cfrac,frac-cfrac+1);
        frac = frac-cfrac+1;
        ii = (1:ns);
        if delay>0
            % data = padarray(data,[0 cfrac 0 0 0 0],0,'pre');
            data = cat(2,zeros(nexc,cfrac,n3,n4,nslices,ncoils),data);
            data = (1-frac)*data(:,ii+1,:,:,:,:) + ...
                frac*data(:,ii,:,:,:,:);
        else
            % data = padarray(data,[0 cfrac 0 0 0 0],0,'post');
            data = cat(2,data,zeros(nexc,cfrac,n3,n4,nslices,ncoils));
            data = frac*data(:,ii+cfrac,:,:,:,:) + ...
                (1-frac)*data(:,ii+cfrac-1,:,:,:,:);
            
        end
        
        % Fourier shift: induces Gibbs ringing
...
if ~isempty(ind_pc)
    if verb, fprintf('Performing a phase correction\n'); end
    for l1=1:nexc
        for l3=1:n3
            for l4=1:n4
                for l5=1:nslices
...
if ~isempty(ind)
    ...
    if isempty(data), error('data empty'); end
    clear dtmp tmp ind
end
...
% Little effect on SNR (even with bw_filt=0.33)
...
%% checks
if any(any(any(any(isnan(out_data)))))
    error('out_data contains NaN'); 
end
if any(any(any(any(isinf(out_data)))))
    error('out_data contains inf');
end
