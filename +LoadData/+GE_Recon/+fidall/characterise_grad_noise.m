...
figid = figure;
posf = get(figid,'position');
ss = get(0,'ScreenSize');
if (posf(2)+780)>ss(4), posf(2) = ss(4)-780; end
set(figid,'position',[posf(1:2) 600 700]);
...
for lax=1:nax
    ...
    if lax==nax
        xlabel(sprintf('freq [kHz] (f0=%g)',...
            round(h.rdb_hdr.ps_mps_freq*1d-4)));
    else
        set(gca,'XTickLabel','');
    end
    set(gca,'YTickLabel','');
...
if sv
    if isdeployed
        print('-dpng',[plt_fname '.png'],'-r300','-painters');
    else
        print('-dpng',[plt_fname '.png'],'-r300');
    end
    saveas(figid,[plt_fname '.fig'],'fig');
end


end      % charactrise_grad_noise.m
