function [k,dcf,t,grad,ind,out,fname]=design_spiral3d(fov,npix,arms,kdt,...
    fname,gmax,smax,nucleus,mode3d,samp_rew,do_rot_file,rewinder,...
    sampfac,do_inout,gdt,save_reduced,save_esp)
...
%    nucleus  Nucleus                                                ('1H')
%     mode3d  Encoding scheme (0=2D; >1->3D rotated spirals)            (0)
%             3D: 1=consecutive, 2=golden angle along radial
%             2D: 0=regular, -1=golden angle (#views=arms(2))
%   samp_rew  Sample rewinder                                       (false)
...
%   save_esp  Save echo-spacing (forbidden zones) into header        (true)
%
...
if ~exist('samp_rew','var'),    samp_rew = []; end
if isempty(samp_rew),           samp_rew = false; end
...
if ~exist('save_esp','var'),    save_esp = []; end
if isempty(save_esp),           save_esp = true; end
...
figure(100); clf;
if kdt==2
    if gdt~=4, error('(kdt==2) && (gdt~=4)'); end
    [~,g1,~,t1] = vds(fufa_smax*smax*1e2,fufa_gmax*gmax*1e2,kdt*1d-6,...
        arms(1)/sampfac(2),fov_mns*1e2*[1,sampfac(3)],1/(2*res_mns*1e2));
    g1 = g1(1,1:2:end);
    t1 = t1(1,1:2:end);
else
    [~,g1,~,t1] = vds(fufa_smax*smax*1e2,fufa_gmax*gmax*1e2,gdt*1d-6,...
        arms(1)/sampfac(2),fov_mns*1e2*[1,sampfac(3)],1/(2*res_mns*1e2));
end
...
%% in-out spiral
if do_inout
    ga_pre = -gdt*1d-6*sum(gspir);
    [gprex,ngx] = gradient_lobe(real(ga_pre),[0 real(gspir(1,end))],...
        fufa_gmax*gmax/sqrt(2),fufa_smax*smax/sqrt(2),gdt*1d-6);
...
%% calculate k-space
kspir = gdt*1d-6*cumsum(gspir);   % [s*T/m]
...
switch rewinder(1)
    case 0         % fast grad ramp down
        nrew = ceil(abs(gspir(1,end))/smax/(gdt*1d-6)/fufa_gmax)+1;
    ...
    case 2         % add crusher
        ...
        [grewx,ngx] = gradient_lobe(real(ga_rew),real(gspir(1,end)),...
            abs(real(gspir(1,end))),fufa_smax*smax/sqrt(2),gdt*1d-6);
        [grewy,ngy] = gradient_lobe(imag(ga_rew),imag(gspir(1,end)),...
            abs(imag(gspir(1,end))),fufa_smax*smax/sqrt(2),gdt*1d-6);
...
if mode3d(1)>0
    ...
    switch mode3d(1)
        ...
        case {2,3}      % golden angle along radial
            ...
            ga = golden_angle(mode3d(2),true);
            phi = mod((0:Ns-1)*ga,360);     % 360° better against artefacts
...
if (mode3d(1)<1)  % regular 2D spirals
    ...
    if mode3d(1)==0
        ...
        % golden angle dcf needs to be regular for QTI encoding
        phitmp = mod(-(0:arms(1)-1)/arms(1)*360/(do_inout+1),360);
        ktmp = bsxfun(@times,kc,exp(-1i*phitmp*pi/180).').';
        dcf = voronoi_area(ktmp(:).'*npix,'','',dcf_thresh);
        % dcf = repmat(dcf(1,1:nk),[1 arms(2)]);
        ddcf = mean(reshape(dcf,[nk arms(1)]).',1);
        if false
            ks = abs(ktmp(:,1)); nks = length(ks);
            dcfs = (ks(2:end,1).^2-ks(1:end-1,1).^2);
            dcfs = (dcfs([1:(nks-1),(nks-1)],1)+dcfs([1,1:(nks-1)],1))/2;
            nn = 20;
            ddcf(1,1:nn) = dcfs(1:nn,1).'/dcfs(nn,1)*ddcf(1,nn);
        end
        dcf = repmat(ddcf,[1 arms(2)]);
    end
    ...
    if ((save_reduced<2) || (nargout>1))
        if mode3d(1)==1
            ...
            k2d = k2d(:).';
            tmp = pi/arms(1)*(k2d(1,2:end).*conj(k2d(1,2:end)) - ...
                k2d(1,1:end-1).*conj(k2d(1,1:end-1)))*npix^2;
            dcf_thresh = max(tmp);
...
acq_pts = ceil(acq_pts/32)*32;             % round to multiple of 32
if acq_pts>16384
...
ts = (0:nk-1)*kdt*1d-6;                    % time of single trajectory [s]
...
if samp_rew   % add info for sampling rewinder
    inds2 = false(1,acq_pts);
    inds2(1,(nk:ceil(ng/dki))) = true;
    ts2 = find(inds2)*kdt*1d-6;
    ks2 = cumsum(grad(:,1,:),1)*gdt*1d-6/2/pi*res_mns*gyrogamma('1H');
    if dki>1
        ks2 = ks2(1:dki:size(ks2,1),1,:)/2/pi*res_mns*gyrogamma('1H');
    end
    if abs(dki-0.5)<1d-10
        ks2 = interp1((1:size(ks2,1)),ks2,(1:0.5:size(ks2,1)+0.5),'spline');
    end
    ks2 = permute(ks2(inds2(1:size(ks2)),1,:),[2 1 3]);

    if ~((mode3d(1)>0) && (save_reduced>0))
        ind2 =  repmat(inds2,[nintlv 1]);
        t2 = repmat(ts2,[nintlv 1]);
        if (mode3d(1)<1)    % 2D 
            k2 = bsxfun(@times,ks2(:,:,1)+1i*ks2(:,:,2),exp(-1i*phi*pi/180).').';
            k2 = k2(:).';
            dcf2 = voronoi_area(k2*npix);
        else
            k2 = zeros(size(ks2,2),nintlv,3);
            kc2 = (ks2(1,:,1)+1i*ks2(1,:,3)).';
            k2(:,:,1) =  real(kc2)*(cosd(theta).*cosd(phi)) - ...
                imag(kc2)*(sind(theta).*cosd(phi));
            k2(:,:,2) =  -real(kc2)*(cosd(theta).*sind(phi)) + ...
                imag(kc2)*(sind(theta).*sind(phi));
            k2(:,:,3) = real(kc2)*sind(theta) + ...
                imag(kc2)*cosd(theta);
            k2 = reshape(k2,[1 length(kc2)*size(phi,2) 3]);
            dcf2 = calc_sdc3(k2,npix);
        end
        dcf2(dcf2>1) = 1;
    end
end
...
smax_act = max(max(abs(diff(gg,[],1))))/(gdt*1d-6);
...
if (islogical(fname) || isnumeric(fname))
    if fname
        switch mode3d(1)
            case -1, str3d = sprintf('2Dga%d_intlv%g',mode3d(2),nintlv); 
            case 0, str3d = '';
        ...
        fname = sprintf('spiral%s_%s_fov%g_mtx%g_arms%g_kdt%g_gmax%g_smax%g_dur%g%s', ...
            str3d,nuc,round(fov*1d3),npix,arms(1),kdt,...
            round(gmax_act*1d3),round(smax_act),...
            round(ng*gdt*1d-2)/10,balstr);
        fname = strrep(fname,'.','p');
        if abs(gdt-4)>1d-10, fname = [fname '_gdt' num2str(gdt)]; end
    else
        fname = [];
    end
end

%% calculate forbidden zone echo-spacing esp
if ~isempty(fname) && save_esp
    [~,~,~,esp_time] = check_grad_acoustics(grad);
else
    esp_time = 0;
end
...
if ~isempty(fname)
    desc = sprintf('design_spiral3D(%g,%g,%g,%g,fname,%g,%g,%s,%g,%g',...
        fov*1d3,npix,arms(1),kdt, gmax*1d3,smax,nuc,mode3d(1),samp_rew);
    desc = sprintf('%s,%g,[%g,%g],[%g,%g,%g,%g],%g,%g,%g,%g)\n',desc,...
        do_rot_file,rewinder(1:2),sampfac(1:4),do_inout,gdt,...
        save_reduced,save_esp);
    ...
    mtx = npix;
    out = write_ak_wav([fname '.wav'],grad,1d6/kdt,fov_mns,desc,acq_pts,...
        esp_time,0,gdt);
    ...
    if ((mode3d(1)>0) && (save_reduced>0))
        ...
        if samp_rew
            ks = ks2; inds = inds2; ts = ts2;
            save([fname '_rew.mat'],'out','ks','ts','inds','nviews','fov','mtx',...
                'npix','gdt','kdt','gmax','smax','nucleus','t_rew','t_arm',...
                'ng_off','gmax_nyquist','rgamma','fov_mns','phi','theta');
        end
    else
        ...
        save([fname '.mat'],'out','k','dcf','t','ind','fov','mtx',...
            ...
            'phi','theta');
        if samp_rew
            k = single(k2); ind = ind2; t = single(t2); dcf = single(dcf2);
            save([fname '_rew.mat'],'out','k','dcf','t','ind','fov','mtx',...
                'npix','gdt','kdt','gmax','smax','nucleus',...
                't_rew','t_arm','ng_off','gmax_nyquist','rgamma','fov_mns',...
                'phi','theta');
        end
...
for l=1:dim
    subplot(dim,1,l);
    plot((1:size(grad,1))*gdt*1d-3,grad(:,:,l)*1d3);
    xlabel('time [ms]');
    ylabel(yl{l})
    grid on
end
