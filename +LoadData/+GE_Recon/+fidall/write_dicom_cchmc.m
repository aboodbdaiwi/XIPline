function hdr=write_dicom_cchmc(fname,b,h,inp,dcmdict,move2dir,verb,slicedim,...
    do_norm)
%WRITE_DICOM  Write GE MR compatible dicom's for importing into database
% hdr = write_dicom(fname,b,h,inp,dcmdict,move2dir,verb,slicedim,do_norm)
%    fname  Filename of dicom image to create
%        b  2D Images to store (max 5D)
%           multislice: slices along (slicedim)th dimension
...
% slicedim  Slice dimension for multislice                              (5)
%  do_norm  Normalise images to 2^15-2                               (true)
%
...
max_scale = 2^15-2;     % normalise to 16bit
inm_fac = 0.999;          % window level: mean of 1-inm_fac times inm_fac
...
if ~exist('slicedim','var'), slicedim = []; end
if isempty(slicedim),        slicedim = 5; end
if (slicedim>5) || (slicedim<3)
    warning('slicedim(=%g)>5 or <3; setting to 5',slicedim);
    slicedim = 5;
end
if ~exist('do_norm','var'),  do_norm = []; end
if isempty(do_norm),         do_norm = true; end
...
if ~isempty(regexpi(hdr.InternalPulseSeqName,'fidall', 'once'))
    is_fidall = true;
else
    is_fidall = false;
end
if ~isempty(regexpi(hdr.InternalPulseSeqName,'3drad', 'once')) || ...
        ~isempty(regexpi(hdr.InternalPulseSeqName,'burzte', 'once'))
    is_zte = true;
else
    is_zte = false;
end
...
%% creating directory
if ~isempty(regexpi(fname,'\.dcm$', 'once')), fname = fname(1:end-4); end
fname = [fname '_dcm'];
...
%% adjust scaling: always use min=0
bvec = sort(abs(b(:)));
maxval = bvec(end);
hdr.UserData16 = maxval;
ii = round(size(bvec,1)*inm_fac):size(bvec,1);
wwidth = mean(bvec(ii))/inm_fac;
clear bvec
if do_norm
    b = int16(b/maxval*max_scale);     % actual rescaling to +- 2^15-2
    wwidth = wwidth/maxval*max_scale;
else
    if maxval>max_scale
        warning('maxval(=%g) > max_scale(=%g); truncating images',...
            maxval,max_scale);
        b(b>max_scale) = max_scale;
    end
    b = int16(b);
end
if any(b(:)<-8)
    warning('b negative; adjusting WindowCenter and WindowWidth');
    wcenter = 0;
    wwidth = floor(2*wwidth);
else
    wcenter = floor(wwidth/2);
    wwidth = floor(wwidth);
end
if ~isfield(hdr,'WindowCenter'), hdr.WindowCenter = wcenter; end
if ~isfield(hdr,'WindowWidth'),  hdr.WindowWidth = wwidth; end
hdr.BitDepth = 16;
...
vec = vec*hdr.SpacingBetweenSlices;    % scale to [mm]
...
if is_zte && ((h.image.plane==4)||(h.image.plane==8)) % 4=sagittal, 8=coronal
...
hdr = sub_add2hdr_dim(hdr,'MRAcquisitionType',h.image.imode,h.rdb_hdr.data_collect_type,verb);
...
hdr_conversion = {...
    ...
    {'AdditionalPatientHistory','exam','hist'},...
    };
end
...
function hdr = sub_add2hdr_dim(hdr,fname,imode,type,verb)
dim_str = '2D';
switch imode       % imaging mode from imaging options
    case {1,4,6} 
    case {2,9}, dim_str = '3D';
    otherwise, warning('unknown imode(=%g): using ''2D''',imode);
end
if bitget(type,7)  % RHTYP3D=64
    if all(imode~=[2,9])
        fprintf('imode(=%g->3D) && data_collect_type(=%g)->3D\n',imode,type);
        fprintf('\tsetting MRAcquisitionType=''3D''\n');
    end
    dim_str = '3D';
end
if ~isfield(hdr,fname)
