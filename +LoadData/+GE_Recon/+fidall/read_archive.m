...
archive = LoadData.GE_Recon.GERecon('Archive.Load', fname);
...
view_offset = [];
if verb>0
    dsize = 8*nframes*frame_size*nphases*nechoes*nslices*nreceivers;
...
for l = 1:archive.ControlCount
    control = LoadData.GE_Recon.GERecon('Archive.Next', archive);
...
if isempty(do_slice_order)
    ...
    if ~isempty(regexpi(psd_iname,'3drad', 'once')) || ...
            ~isempty(regexpi(psd_iname,'burzte', 'once'))
...
if nargout>1
    h = LoadData.GE_Recon.fidall.archive2header(archive,verb>2);
    for lslice=1:nslices
        arcInfo = LoadData.GE_Recon.GERecon('Archive.Info', archive, lslice);
...
if any(any(any(any(any(any(isnan(d)))))))
    % if any(isnan(d(:)))
    warning('data contains NaN; replacing by 0');
    d(isnan(d)) = 0;
end
if any(any(any(any(any(any(isinf(d)))))))
    % if any(isinf(d(:)))
