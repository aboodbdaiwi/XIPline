...
1. matrix Gx x Gy, complex
2. trajectory 3 x nPoints
3. [FOVx FOVy] doubles
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
  ...
  Gx_i = mxGetM(prhs[0]);
  Gy_i = mxGetN(prhs[0]);
  matrix_real = mxGetPr(prhs[0]);
  matrix_imag = mxGetPi(prhs[0]);
  ...
  /* Reserve space for interpolated data */
  plhs[0] = mxCreateDoubleMatrix(trajectory.length, 1, mxCOMPLEX);
  data_real = mxGetPr(plhs[0]);
  data_imag = mxGetPi(plhs[0]);


  /* Initialize our kernel */
  lk.C[0] = (float *)malloc(4096);
  /*  posix_memalign( (void**)lk.C, 4096, 4096 ); */
  lk.C[1] = lk.C[0] + 512;
  slot = 0;
  ...
  for ( iPoint = 0; iPoint < trajectory.length; iPoint++ ) {
    ...
    /* zero out this datum */
    data_real[iPoint] = 0.0;
    data_imag[iPoint] = 0.0;


    /* Make sure this point and kernel are on the gridMatrix */
    if ( gridMatrix_x_i >= 0 && (gridMatrix_x_i + 3) < Gx_i && gridMatrix_y_i >= 0 && (gridMatrix_y_i + Wy-1) < Gy_i ) {
      /* Weights Calculation */
      ...
      double * local_grid_real = matrix_real + (Gx_i * gridMatrix_y_i + gridMatrix_x_i);
      double * local_grid_imag = matrix_imag + (Gx_i * gridMatrix_y_i + gridMatrix_x_i);
      int iy;
      ...
      /* Perform inverse gridding*/
      for ( iy = 0; iy < Wy; iy++ ) {
        data_real[iPoint] += local_grid_real[0] * yWeight[iy] * xWeight[0];
        data_imag[iPoint] += local_grid_imag[0] * yWeight[iy] * xWeight[0];
        
        data_real[iPoint] += local_grid_real[1] * yWeight[iy] * xWeight[1];
        data_imag[iPoint] += local_grid_imag[1] * yWeight[iy] * xWeight[1];
	
        data_real[iPoint] += local_grid_real[2] * yWeight[iy] * xWeight[2];
        data_imag[iPoint] += local_grid_imag[2] * yWeight[iy] * xWeight[2];
	
    	data_real[iPoint] += local_grid_real[3] * yWeight[iy] * xWeight[3];
        data_imag[iPoint] += local_grid_imag[3] * yWeight[iy] * xWeight[3];

        local_grid_real += Gx_i;  /* skip to next line */
        local_grid_imag += Gx_i;
      } /* for iy */
    } /* if */
  } /* Point Loop */
