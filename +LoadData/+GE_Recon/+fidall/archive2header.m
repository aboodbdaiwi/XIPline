...
if rdcsz
    ...
    h.data_acq_tab = h.data_acq_tab(1:h.rdb_hdr.nslices);
end


%% convert data_acq_tab to match Matlab + Ox-pfile reading routines
flds = fieldnames(h.data_acq_tab);
nn = length(h.data_acq_tab);
for l1=1:length(flds)
    for l2=1:nn
        ent = h.data_acq_tab(l2).(flds{l1});
        switch length(ent)
            case 1, dat.(flds{l1})(l2,1) = ent;
            case 3, dat.(flds{l1})(l2,1,:) = reshape(ent,[1 1 3]);
            otherwise
                warning('length(h.data_acq_tab(%d).%s)(=%d)~=1 or 3',...
                    l2,(flds{l1}),length(ent));
        end
    end
end
h.data_acq_tab = dat;
