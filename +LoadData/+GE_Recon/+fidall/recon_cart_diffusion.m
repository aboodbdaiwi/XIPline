function [bb,bbabs] = recon_cart_diffusion(d,h,wfn,zf,delay,lb,fname,comb_time)
%RECON_CART Reconstruct Cartesian readout acquired with fidall
% [bb,bbabs] = recon_cart_diffusion(d,h,wfn,mtx,delay,lb,fname,comb_time)
%                                                               (default)
...
fina = {'k','dcf','ind','t','npix','mtx','fov','sampfac','dim','bvalues'};
for l=1:length(fina)
...
tmp = wf.k(1,:,1:2);
tmp = reshape(tmp,sum(wf.ind(1,:)),2,size(d,1)/nbvalues,2);
...
if csi_dims==0
    ...
    dd = complex(zeros(1,ns1,nslices,nrep,ncoils,nbvalues,class(d)));
    
    for lb = 1:nbvalues
        for lr=1:nrep
            for lsli=1:nslices
                for lc=1:ncoils
                    dd(1,:,lsli,lr,lc,lb) = dtmp(lc,:,lr,lsli,lb);
                end
            end
        end
    end
...
dd = reshape(dd,[nk_freq nk_pha1 nk_pha2 nrep ncoils nbvalues]);
...
if trnsps(1)==3, ntmp = ntmp(1,[2,1,3,4,5,6]); end
...
for lb = 1:nbvalues
    for lt=1:nrep
        for lc=1:ncoils
            for l3=1:n3
                if trnsps(1)==3
                    bbtmp(:,:,l3,lt,lc,lb) = rot90(permute(fliplr(bb(:,:,l3,lt,lc,lb)),...
                        [2 1 3]),rot90fac(1));
                else
                    bbtmp(:,:,l3,lt,lc,lb) = rot90(fliplr(bb(:,:,l3,lt,lc,lb)),rot90fac(1));
                end
            end
        end
    end
...
switch dim
    case 2
        for lb = 1:nbvalues
            ...
            bbplt = reshape(bbabs(:,:,:,:,lb),[size(bbabs,1),size(bbabs,2),n3,nrep]);
            bbplt = permute(bbplt,[1 2 4 3]);
            imagesc_row(bbplt,[],[],false,true);
            
            colormap(gray);
            figstr = sprintf('P%05d Exam%d Series%d_Bval%d',...
                h.image.rawrunnum,h.exam.ex_no,h.series.se_no,lb);
            set(gcf,'name',figstr);
            if ~isempty(fname)
                print([fname '_bval' num2str(lb)],'-dpng','-r600');
            end
        end
    case 3
        for lb = 1:nbvalues
            figure; set(gcf,'DefaultAxesFontSize',14);
            imagesc_ind3d(bbabs(:,:,:,:,lb),[],[],[],false,true);
            colormap(gray);
            figstr = sprintf('P%05d Exam%d Series%d_Bval%d',...
                h.image.rawrunnum,h.exam.ex_no,h.series.se_no,lb);
            set(gcf,'name',figstr);
...
if ~isempty(fname)
    ...
    tmp = sprintf('MNSRP;CART;SL%g,TS%g,BV%g',nslices,nrep,nbvalues);
    tmp = [tmp ';' h.series.se_desc];
    inp.SeriesDescription = tmp;
    write_dicom(fname,squeeze(bbabs),h,inp);
end
