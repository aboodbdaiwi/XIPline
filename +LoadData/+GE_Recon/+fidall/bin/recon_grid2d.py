...
if len(sys.argv)>6:
  ...
  fnlog = fidalldir + 'log/' + now.strftime("%Y%m%d_%H%M%S") + '_recon_grid2d.txt'
...
cmd = ['recon_spiral',datadir+rawdataname,'[]',wfn,mtx,delay,lbt,spks,datadir+rawdataname]
sor.run(cmd,mascri)

# finishing up
print('recon_grid2d.py done')
if len(sys.argv)>6:
  sys.stdout.close()
