...
lb = '0'                     # Gaussian linebroadening [Hz]


# redirect output to logfile
if len(sys.argv)>6:
  now = datetime.datetime.now()
  fnlog = fidalldir + 'log/' + now.strftime("%Y%m%d_%H%M%S") + '_recon_concat.txt'
...
cmd = ['recon_concat',datadir+rawdataname,'[]',wfn,datadir+rawdataname,delay,mtx,lb]
sor.run(cmd,mascri)

# finishing up
print('recon_concat.py done')
if len(sys.argv)>6:
  sys.stdout.close()
