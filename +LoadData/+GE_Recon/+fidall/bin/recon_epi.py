...
if len(sys.argv)>6:
  ...
  fnlog = fidalldir + 'log/' + now.strftime("%Y%m%d_%H%M%S") + '_recon_epi.txt'
...
cmd = ['recon_epi',datadir+rawdataname,'[]',wfn,mtx,delay,datadir+rawdataname]
sor.run(cmd,mascri)

# finishing up
print('recon_epi.py done')
if len(sys.argv)>6:
  sys.stdout.close()
