...
# -*- coding: utf-8 -*-
"""
Reconstruct Cartesian CSI B1+ Maps
Created: 8/2022
@author: Rolf Schulte
"""
...
zf = '[]'                    # zero-filling
lb = '[]'                    # apodisation [s,x,y,z]

# redirect output to logfile
if len(sys.argv)>6:
  now = datetime.datetime.now()
  fnlog = fidalldir + 'log/' + now.strftime("%Y%m%d_%H%M%S") + '_recon_csi_blosi.txt'
...
cmd = ['recon_csi_blosi',datadir+rawdataname,'[]',zf,lb,datadir+rawdataname]
sor.run(cmd,mascri)

# finishing up
print('recon_csi_blosi.py done')
if len(sys.argv)>6:
  sys.stdout.close()
