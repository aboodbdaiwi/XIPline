...
if len(sys.argv)>6:
  ...
  fnlog = topdir + 'log/' + now.strftime("%Y%m%d_%H%M%S") + '_recon_csi.txt'
...
cmd = ['recon_csi',datadir+rawdataname,'[]',zf,lbt,datadir+rawdataname,plt,coco,ws]
sor.run(cmd,mascri)

# finishing up
print('recon_csi.py done')
if len(sys.argv)>6:
  sys.stdout.close()
