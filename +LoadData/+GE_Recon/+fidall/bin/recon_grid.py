...
# -*- coding: utf-8 -*-
"""
2D/3D gridding reconstruction (+ stack-of-2D)
7/2025
@author: Rolf Schulte
"""
...
if len(sys.argv)>6:
  ...
  fnlog = fidalldir + 'log/' + now.strftime("%Y%m%d_%H%M%S") + '_recon_grid.txt'
...
cmd = ['recon_grid',rawin,'[]',wfn,mtx,delay,lbt,datadir+rawdataname,mask_fac,coco,plt,grdwrp]
if not use_vre:
  # on host
  if dofifo:
    sor.run_fifo(cmd)
