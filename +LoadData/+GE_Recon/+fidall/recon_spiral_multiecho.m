...
%export_dcm  Save images in dicom format             isdployed->(true)
%
...
if ~isnumeric(d)
    ...
    [d,h] = read_p(d, true, true);       % uses single for d>1GB
end
...
if isstruct(wfn)
    ...
    wf = load_waveform(wfn);          % load waveform
end

%% read number of echoes from wf
if isfield(wf, 'te')
    num_echoes = length(wf.te);
else
    num_echoes = 1;
end
...
if isfield(wf,'npix'), mtx_acq = wf.npix;
elseif isfield(wf,'mtx') 
    mtx_acq = wf.mtx(:).';       % nominal matrix resolution of acquisition
else
    error('wf.npix not existing'); 
end
...
% nominal matrix resolution of acquisition
...
if abs(mtx_reco(1))<1
    ...
    kscale = mtx_acq./mtx_reco;
    kscale = kscale(1:size(k,3));
    k = bsxfun(@times,k,shiftdim(kscale,-1));
end
...
% tmp = false(size(wf.ind,1), size(wf.ind,2)+2); 
% tmp(:,1:end-3) = wf.ind(:,2:end); 
% wf.ind = tmp; 

if isfield(wf, 'nk_pha')
    d = d(:, wf.nk_pha+1:end, :, :, : ,:); 
    wf.ind = wf.ind(:, wf.nk_pha+1:end);
end


d = d(:,1:size(wf.ind,2)*num_echoes, :, :, :,:);
d = reshape(d, size(d,1), size(wf.ind,2), num_echoes, size(d,3), size(d,4), size(d,5), size(d,6));
d = permute(d, [1 2 4 5 6 7 3]); 
d = reshape(d, size(d,1), size(d,2), size(d,3),size(d,4), size(d,5), size(d,6)*size(d,7));
% tmp = reshape(tmp, ncoils, size(wf.t,2)*size(wf.t,1), ntimesteps, nslices, num_echoes);
% dd= tmp;

dd = raw2grid(d,ischop(h),k,shft,cart_down_fac,[],wf.ind,delay*1d-6,bw,...
    false,ind_pc);

dd = reshape(dd, ncoils, num_echoes, size(dd,2), size(dd,3), size(dd,4));
dd = permute(dd, [1 3 4 5 2]);

if isfield(wf, 'kz') %check if phase encoding along third dimension (z)
    pha_encodes = single(unique(wf.kz));
    num_arms= wf.nviews/ length(pha_encodes);
    nslices = length(pha_encodes);
    tmp_dd = zeros(size(dd,1), size(dd,2)/length(pha_encodes), size(dd,3),length(pha_encodes), size(dd,5));
    tmp_k = zeros(1, size(k,2)/length(pha_encodes),3,length(pha_encodes));
    for i=1:length(pha_encodes)
        id = k(:,:,3)==pha_encodes(i);
        tmp_dd(:,:,:,i,:)= dd(:,id,:,:,:);
        tmp_k(:,:,:,i) = k(:,id,:);
    end
    k = k(:,id,1:2);
    ...
    dd = tmp_dd;
    clear tmp_dd;
    dd = fftshift(fft(fftshift(dd,4),[],4),4);
end
    ...
    if fidfirst
        ...
        if plt || ~isempty(fname)
            for l5=1:size(spec,5)
                ...
                set(fid,'name',figstr);
                if ~isempty(fname)
                    print([fname '_spec_ls' num2str(l5) '.png'],'-dpng',...
                        '-r300',renderer);
                    if export_dcm>0
...
bb = zeros(mtx_reco(1),mtx_reco(2),ntimesteps,nmeta,nslices,ncoils, num_echoes);

%% actual reconstruction
for le = 1:num_echoes
    for lt=1:ntimesteps
        for ls=1:nslices
            if strcmpi(reco,'normal')
                b1 = gridding(dd(:,:,lt,ls,le),k,wf.dcf,mtx_reco(1:2));
                % b1 = grid_matlab(dd(lc,:,lt,ls),k,mtx_reco);
                for lc=1:ncoils
                    if trnsps(1)==3
                        bb(:,:,lt,1,ls,lc,le) = rot90(fliplr(b1(:,:,lc)).',rot90fac(1));
                    else
                        bb(:,:,lt,1,ls,lc,le) = rot90(fliplr(b1(:,:,lc)),rot90fac(1));
                    end
                end
                ...
                for lc=1:ncoils
                    switch reco
                        ...
                        case 'spsp'
                            for lm=1:nmeta
                                ll = lm + (lt-1)*nmeta;
                                b1 = gridding(dd(lc,:,ll,ls,le),k,wf.dcf,mtx_reco);
                                if trnsps(1)==3
                                    bb(:,:,lt,lm,ls,lc,le) = rot90(fliplr(b1).',rot90fac(1));
                                else
                                    bb(:,:,lt,lm,ls,lc,le) = rot90(fliplr(b1),rot90fac(1));
                                end
                            end
                        ...
                        case 'ideal'
                            ...
                            d1 = reshape(((pinv_A*squeeze(dd(lc,:,ind_t,ls,le)).').* ...
            ...
            end        % if strcmpi(reco,'normal')
        end       % for ls=1:nslices
    end     % for lt=1:ntimesteps
end    % for le=1:num_echoes
clear dd

%% combine multiechoe dim and timestep dim
bb = permute(bb, [1 2 7 3 4 5 6]);
bb = reshape(bb, size(bb,1), size(bb,2),size(bb,3)*size(bb,4), size(bb,5), size(bb,6), size(bb,7));
ntimesteps = ntimesteps*num_echoes;
...
if plt || ~isempty(fname)
    if nmeta==1
        ...
        if (ntimesteps>10) && (nslices==1), rshp = true; end
        set(fid,'DefaultAxesFontSize',14);
        imagesc_row(reshape(bbabs,[mtx_reco(1),mtx_reco(2),ntimesteps,nslices]),...
            [],'ind',rshp,true);
        colormap(gray);
        ...
        set(fid,'name',figstr);
        if ~isempty(fname)
            print([fname '.png'],'-dpng','-r300',renderer);
        end
        if ~plt, close(fid); end
    else
        ...
        for l5=1:nslices
            ...
            set(fid,'name',figstr);
            if ~isempty(fname)
                print([fname '_ls' num2str(l5) '.png'],'-dpng','-r300',renderer);
            end
            if ~plt, close(fid); end
        end
    end
...
%% info
fprintf('recon_spiral: runtime = %.2f [s]\n',toc(timerVal));
