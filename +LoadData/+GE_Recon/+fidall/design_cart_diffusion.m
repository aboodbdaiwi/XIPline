function [grad,ind] = design_cart_diffusion(fov,mtx,bw,gmax,smax,fname,blncd,...
...
%     b_vals  b-values to use                                       (0, 12)
%  diff_time  length of diffusion encoding              [ms]            (5)
%   diff_gap  time between gradient lobes               [ms]            (0)
%
...
%
%  9/2024 Rolf Schulte
%  2/2025 Matt Willmering - diffusion modifications
...
if ~exist('do_vap','var'), do_vap = []; end
if isempty(do_vap),        do_vap = true; end
if ~exist('inv_grad','var'), inv_grad = []; end
if isempty(inv_grad),      inv_grad = false; end
if ~exist('b_vals','var'), b_vals = []; end
if isempty(b_vals),      b_vals = [0, 12]; end
if ~exist('diff_time','var'), diff_time = []; end
if isempty(diff_time),      diff_time = 5; end  
if ~exist('diff_gap','var'), diff_gap = []; end
if isempty(diff_gap),      diff_gap = 0; end
...
fov = [fov , ones(1,3-dim)];           % add 1's for grad pre-allocation
fov = fov*1d-3;                        % [mm] -> [m]
num_b = length(b_vals);
diff_time = diff_time*1d-3;            % [ms] -> [s]
diff_gap = diff_gap*1d-3;            % [ms] -> [s]

%% diffusion encoding
% determine timing
n_ramp = ceil(gmax(1)/smax(1)/gdt);   % #ramp pts maximum
tslope = n_ramp*gdt; % [s]
n_plat = ceil((diff_time-(2*tslope))/gdt);
if n_plat < 1
    warning('n_plat(=%s) !> 0; calculations not set up for triangular diffusion grads',n_plat);
end
tlenc = n_plat*gdt;
n_gap = ceil(diff_gap/gdt);
n_diff_grad_pts = 4*n_ramp+2*n_plat+n_gap;
d = 2*tslope+tlenc; % little delta
D = d+diff_gap; % big delta

% determine g strength(s)
gXe = 0.74e8;      % [s^-1·T^-1]         
% bvalueXe=(gXe*Gm).^2*(d^2*(D-d/3) + b*(d^2 - 2*D*d + D*b - 7*d*b/6 + 8/15*b^2))*1e-4 %Abood eqn
% bvalueXe=gXe.^2*Gm.^2*k*1e-4 where k=(d^2*(D-d/3) + b*(d^2 - 2*D*d + D*b - 7*d*b/6 + 8/15*b^2))
% Gm = sqrt(bvalueXe/(gXe.^2*k*1e-4))
k=(d^2*(D-d/3) + tslope*(d^2 - 2*D*d + D*tslope - 7*d*tslope/6 + 8/15*tslope^2));
Gm=zeros(size(b_vals));
diff_grads = zeros(num_b,n_diff_grad_pts);
for b=1:length(b_vals)
    b_val = b_vals(b);
    Gm(b) = sqrt(b_val/(gXe.^2*k*1e-4));
    diff_grads(b,:) = [linspace(0,Gm(b),n_ramp)... %ramp pos
                       Gm(b)*ones(n_plat,1)'...     %plat
                       linspace(Gm(b),0,n_ramp)... %ramp down
                       zeros(n_gap,1)'...           %gap
                       linspace(0,-Gm(b),n_ramp)... %ramp neg
                       -Gm(b)*ones(n_plat,1)'...     %plat
                       linspace(-Gm(b),0,n_ramp)... %ramp down
                       ];
end
...
if do_vap
    if dda>0,  error('do_vap && dda(=%g)>0', dda); end
    if gdt2>4, error('do_vap && gdt2(=%g)>4',gdt); end
    grad1 = zeros(n_grd,1,dim);
end

grad = zeros(n_grd,mtx(2)*mtx(3),dim);
grad(:,:,1) = repmat(gfreq.',[1 mtx(2)*mtx(3)]);
if do_vap, grad1(:,1,1) = gfreq.'; end
if dim>1
    ii1 = (1:n_pha1);
    ...
    if do_vap
        grad1(ii1,1,2) =  G_pha1;
        if blncd, grad1(ii2,1,2) = -G_pha1; end
    end
end
if dim>2
    ii1 = (1:n_pha2);
    ...
    if blncd, grad(ii2,:,3) = -gpha2; end
     if do_vap
        grad1(ii1,1,3) =  G_pha2;
        if blncd, grad1(ii2,1,3) = -G_pha2; end
    end
end
...
%% Duplicate exc and add diff grads
diff_grad = zeros(size(grad,1)+size(diff_grads,2), size(grad,2)*size(b_vals,2), 3); % hardcode to 3 axis
for e=1:size(grad,2)
    for b=1:size(b_vals,2)
        diff_grad(:,b+length(b_vals)*(e-1),1) = [zeros(size(diff_grads,2),1)',...
            grad(:,e,1)'];
        diff_grad(:,b+length(b_vals)*(e-1),2) = [zeros(size(diff_grads,2),1)',...
            grad(:,e,2)'];
        diff_grad(:,b+length(b_vals)*(e-1),3) = [diff_grads(b,:),...
            zeros(size(grad,1),1)'];
    end
end
n_grd = n_grd + n_diff_grad_pts;
gfreq = diff_grad(:,1,1);
grad = diff_grad;
...
ii = n_goff+n_rew+n_diff_grad_pts+(1:n_plat)-1;   % index
ind_grad(ii,1) = true;
inds = false(n_ind,1);             % acq index list (kdt)
inds(1:ceil(n_grd/dki),:) = ind_grad(1:dki:n_grd,1);


%% k-space trajectory (for recon_spiral.m and recon_grid3d.m)
kk = cumsum(grad,1)*gdt*abs(gyrogamma(nucleus));
k = kk(1:dki:end,:,:);
k = k(inds(1:size(k,1),1),:,:);
k = reshape(k,[1 size(k,1)*size(k,2) size(k,3)]);
k = bsxfun(@rdivide,k,2*pi*reshape(mtx./fov,[1 1 3]));

ts = (0:n_ind-1)/bw;
ts = ts(1,inds);
ind = repmat(inds,[1 mtx(2)*mtx(3)]).';
t = repmat(ts,[1 mtx(2)*mtx(3)]);

%% add dummy scans
if dda>0
    ind = [false(dda,size(ind,2)) ; ind];
    grad = [zeros(size(grad,1),dda,dim) , grad];
    nexc = mtx(2)*mtx(3)*size(b_vals,2) + dda;
else
    nexc = mtx(2)*mtx(3)*size(b_vals,2);
end

%% print info
fprintf('BW=%g [kHz] (full)=+-%g [kHz] (imaging)\n',bw*1d-3,bw*0.5d-3);
fprintf('g_read=%g [mT/m]; n_plat=%g; n_rew=%g; n_ramp=%g; n_diff=%g\n',...
    g_read*1d3,n_plat,n_rew,n_ramp,n_diff_grad_pts);
...
subplot(3,3,1); %mag, cum, slew and x, y, z
...
if size(grad,3)>1
    ...
    subplot(3,3,5); 
    csgf = cumsum(grad(:,i2,2))*gdt;
    plot(tg,csgf,'b'); 
    ylabel('sum grad'); grid on; 
    axis([0 dur -1.1*max(abs(csgf)) 1.1*max(abs(csgf))]);
    subplot(3,3,8); 
    plot(tg(1:end-1),diff(grad(:,i2,2))/gdt,'b',...
        [0 dur],smax(1)*[1 1],'m--',[0 dur],-smax(1)*[1 1],'m--');
    ylabel('slewrate [T/m/s]'); grid on; axis([0 dur -1.1*smax(1) 1.1*smax(1)]);
    xlabel('time [ms]');
end
...
if size(grad,3)>2
    subplot(3,3,3);
    [~,ii] = max(reshape(grad(:,:,3),[size(grad,1)*size(grad,2) 1]));
    [~,i2] = ind2sub([size(grad,1) size(grad,2)],ii);
    plot(tg,grad(:,i2,3)*1d3,'b',...
        tg(ind_grad),grad(ind_grad,1,2)*1d3,'rx',...
        [0 dur],gmax(2)*[1d3 1d3],'m--',[0 dur],-gmax(2)*[1d3 1d3],'m--');
    ylabel('G-sl [mT/m]'); grid on; axis([0 dur -gmax(2)*1.1d3 gmax(2)*1.1d3]);
    subplot(3,3,6); 
    csgf = cumsum(grad(:,i2,3))*gdt;
    plot(tg,csgf,'b'); 
    ylabel('sum grad'); grid on; 
    axis([0 dur -1.1*max(abs(csgf)) 1.1*max(abs(csgf))]);
    subplot(3,3,9); 
    plot(tg(1:end-1),diff(grad(:,i2,3))/gdt,'b',...
        [0 dur],smax(1)*[1 1],'m--',[0 dur],-smax(1)*[1 1],'m--');
    ylabel('slewrate [T/m/s]'); grid on; axis([0 dur -1.1*smax(1) 1.1*smax(1)]);
    xlabel('time [ms]');
end
...
if (islogical(fname) || isnumeric(fname))
    if fname
        ...
        fname = sprintf('diffcart%gD_%s_fov%s_mtx%s_nexc%d_kdt%g_gmax%g_smax%g_dur%s', ...
        ...
        if inv_grad, fname = [fname '_ginv']; end
    else
        fname = [];
    end
end
...
if ~isempty(fname)
    ...
    npix = max(mtx);
    if length(mtx)>dim, mtx = mtx(1,1:dim); end
    ...
    if do_vap
        out = write_ak_wav([fname,'.wav'],grad1,bw,fov1H,desc,n_ind,[],[],gdt2);
        if dim>1, write_fdl([fname '_agy.fdl'],x1(:).','agrad'); end
        if dim>2, write_fdl([fname '_agz.fdl'],x2(:).','agrad'); end
    else
    ...
    dcf = ones(1,prod(mtx)*sampfac(1)*num_b,class(k))/sampfac(1);
    if ~exist('ind_pe','var'), ind_pe = []; end
    bvalues = b_vals; % set to match diff recon from A. Hahn
    save(fname,'out','ind','fov','fov1H','mtx','gmax','smax','nucleus',...
        'npix','n_ind','t','k','dcf','sampfac','dim','dda','ind_pe',...
        'inv_grad','bvalues');
end

end      % design_cart_diffusion.m
