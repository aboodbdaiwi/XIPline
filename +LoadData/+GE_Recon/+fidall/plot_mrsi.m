...
% Last Modified by GUIDE v2.5 26-Aug-2021 11:53:18
...
spec = spec(end:-1:1,:,:,:,:);         % for reverse axis

if length(size(spec))<3, error('spec<3D'); end
n1 = size(spec,1);                     % #spectral points
spec = spec/max(abs(spec(:)));         % normalise max to 1
if size(spec,5)>1
    spec = cat(5,spec,mean(spec,5));   % add averaged spectrum to end
end
handles.spec = spec;                   % attach mrsi to handles
if isfield(par,'pc')                   % input phase correction
    handles.pc = par.pc;
else
    handles.pc = 0;
end
if isfield(par,'lb')                   % input apodisation
    if isempty(par.lb)
        lb = 0; 
    else
        lb = par.lb(1);
...
handles.fax = fax;                % freq axis
handles.df = abs(fax(2)-fax(1));  % freq discretisation
handles.show_line1 = false;       % show lines only after moving slider
handles.show_line2 = false;
if isfield(par,'fig_str')         % prepend text to figure
    fig_str = [par.fig_str ': '];
else
    fig_str = '';
end
...
sub_adjust_sliders(handles,verb); % adjust sliders with Min,Max,Value
handles.output = hObject;         % default output for plot_mrsi
guidata(hObject, handles);        % update handles structure
figure(handles.figure1);          % open figure
sub_plot_mrsi(handles,verb);      % actual plotting

end      % plot_mrsi_OpeningFcn
...
function slider_ind5_Callback(hObject, eventdata, handles)
sub_slider(hObject,1);
sub_plot_mrsi(handles);
end
...
if do_abs
    ...
    % set(handles.text_pc,'Visible','off');
else
    set(handles.slider_pc,'Visible','on');    
    % set(handles.text_pc,'Visible','on');
end
sub_plot_mrsi(handles);
end
...
timestep = round(get(handles.slider_ind5,'Value'));
...
spec = handles.spec(min1:max1,min2:max2,min3:max3,slice,timestep);
[n1,n2,n3] = size(spec);
% line-broadening
...
if abs(lb)>0.01
    ...
    fid = ifft(fftshift(spec(end:-1:1,:,:,:,:),1),[],1);
...
set(handles.figure1,'Name',fig_str);
...
[n1,n2,n3,n4,n5] = size(handles.spec);
...
if verb, fprintf('Setting slider_ind5'); end
if n5==1
    set(handles.slider_ind5,'Visible','off');
    set(handles.text_ind5,'Visible','off');
else
    set(handles.slider_ind5,'Visible','on');
    set(handles.text_ind5,'Visible','on');
    if get(handles.slider_ind5,'Max')~=n5
        set(handles.slider_ind5,'Max',n5);
        set(handles.slider_ind5,'Value',n5);
    end
    set(handles.slider_ind5,'TooltipString',num2str(n5));
    set(handles.slider_ind5,'SliderStep',[1 1]/(n5-1));
end

if verb, fprintf('Done with sub_adjust_sliders\n'); end

end      % sub_adjust_sliders
