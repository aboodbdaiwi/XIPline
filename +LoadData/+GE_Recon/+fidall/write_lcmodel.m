...
if isempty(dim_spec),        [~,dim_spec] = max(size(data)); end
...
if isempty(fname)
    ...
    if ~isempty(regexpi(fname,'\.raw$', 'once')), fname = fname(1:end-4); end
...
if ~isempty(basis)
    ...
    fprintf('No basis file specified: skipping control file\n');
end