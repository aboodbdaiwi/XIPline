...
%  resmpl   Truncate/zero-fill Fourier transformed data                 (0) 
%           0=off, 1=ifftn/fftn, 2=fftn/ifftn
...
if isempty(resmpl),          resmpl = 0; end
...
if verb && (min(new_mtx)<2), fprintf('min(new_mtx)(=%g)<2\n',min(new_mtx)); end

for l=1:10,  ind{l} = 1; end                % initialise index list
for l=1:dim, ind{l} = (1:new_mtx(l))+floor((mtx(l)-new_mtx(l))/2); end
switch resmpl      % downsampling
    case 0
    case 1, d = ifftshift(ifftn(d));
    case 2, d = ifftshift(fftn(d)); 
    otherwise
        warning('resampl(=%g)~=0,1,2',resmpl);
end
...
if exist('zf','var')
    ...
    if isreal(d)
        dtmp = zeros(zf,class(d));
    else
        dtmp = complex(zeros(zf,class(d)));
    end
...
if resmpl
    ...
    switch resmpl      % FFT back
        case 0
        case 1, d = fftn(fftshift(d));
        case 2, d = ifftn(fftshift(d));
        otherwise
            warning('resampl(=%g)~=0,1,2',resmpl);
    end
