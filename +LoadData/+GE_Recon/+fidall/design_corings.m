...
if ntmp>ng_max
    warning('#grad points(=%d) exceeding fidall limit(=%d); decrease gdt',...
        ntmp,ng_max);
end
if verb>0
...
switch scheme    
    ...
    case 2    % density-weighted Hanning
        ...
        [~,~,rad] = sub_dw(mtx(1),1,sampfac(2),alpha(1),beta(1),verb);
        rad = [0 rad];
        rad = rad/max(rad)*0.5;
        warning('normalising rad: real~=nominal resolution');
        n_ring = size(rad,2);
        apo = (1+cos(pi*(0:(n_ring-1))/n_ring/alpha(1)))/2; % apodisation

        if dim>2
            kz = sub_dw(mtx(3),1,sampfac(3),alpha(1),beta(1),verb);
            kz = kz/max(abs(kz))*0.5;
            warning('normalising kz: real~=nominal resolution');
            n_zpe = size(kz,2);                  % #z-phase encodes
        else
            n_zpe = 1;
        end
        
    case 22    % OLD: density-weighted Hanning
        % calculate k-space positions in radial direction
...
if isodd(size(grad,1)), grad = [grad ; zeros(1,size(grad,2),2)]; end
if n_intlv>1
    % adding spectral interleaves -> rotating waveforms
    gtmp = zeros(size(grad,1),n_ring*n_intlv,2);
    for li=1:n_intlv
...
if dim>2
    rr = 2*repmat(kz,[n_ring*n_intlv 1]); rr = rr(:).';
...
dcf1 = voronoi_area(k1*mtx(1),1,3);
...
if verb>1
    ...
    title('k-space (spatial)');

    
    figure(102); clf
    plot3(t*1d3,k(1,:,1),k(1,:,2),'x');
    title('k-space (spatial+spectral)');
...
end      % design_corings.m


%% sub-function (from design_mrsi.m)
function [k,dcf,rad] = sub_dw(mtx,dim,samp,alpha,beta,verb)
if any(diff(mtx)~=0)
