...
%      coco  Coil combination via mri_coil_combine.m           ([2 0 0 16])
...
if ~isnumeric(d) || isempty(d)
    ...
    [d,h] = read_p(d,true);       % single precision 
end
...
if isstruct(wfn)
    ...
    if isempty(regexpi(wfn,'kspfile', 'once')) && ...
            isempty(regexpi(wfn,'fidspiral.grad', 'once'))
        ...
        if ~isempty(regexpi(wfn,'kspfile', 'once'))
            % load kspfile k-space file from (fid)spiral.e
            if ~exist(wfn,'file'), error('file not found: wfn=%s',wfn); end
            wf = kspace2waveform(wfn,h);
            if size(d,1) ~= h.image.dim_Y       % remove the two b0map frames
                fprintf('size(d,1)(=%d) ~= h.image.dim_Y(=%d): skipping first 2 frames\n',...
                    size(d,1),h.image.dim_Y);
                if size(d,1) == h.image.dim_Y+2
                    d = d(3:end,:,:,:,:,:);
                else
                    warning('size(d,1) ~= h.image.dim_Y+2: cannot truncate');
                end
            end
        else
            % fidspiral.grad gradient waveform file
            wf = grad2waveform(wfn,h);
        end
    end
end
...
if isempty(freqs)
    if h.rdb_hdr.user40>0
        ...
        switch reco
            ...
            otherwise, warning('cvfname'); cvfname = [];
        end
...
bw = h.rdb_hdr.user0;
if abs(bw)<1, bw = h.rdb_hdr.bw*2d3; end         % for spiral.e
...
for lt=1:ntimesteps
    for ls=1:nslices
        if strcmpi(reco,'normal')
            b1 = gridding(dd(:,:,lt,ls),k,wf.dcf,mtx_reco);
            % b1 = grid_matlab(dd(lc,:,lt,ls),k,mtx_reco);
            for lc=1:ncoils
                if trnsps(1)==3
                    bb(:,:,lt,1,ls,lc) = rot90(fliplr(b1(:,:,lc)).',rot90fac(1));
                else
                    bb(:,:,lt,1,ls,lc) = rot90(fliplr(b1(:,:,lc)),rot90fac(1));
                end
            end
...
if ncoils>1
    ...
    if coco(1)>-1
        ...
        % bb = mri_coil_combine(bb,coco(2),'',coco(1));
...
if ~isempty(fname)
    ...
    save([fname '.mat'],'-mat','-v7.3',...
        'bb','h','wfn','mtx_reco','delay','lbt','spks','fname',...
        'cart_down_fac','metstr','bbabs','ss','fax');
end
