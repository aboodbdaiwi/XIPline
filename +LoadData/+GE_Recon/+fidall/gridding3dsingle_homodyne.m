function [img, gdat] = gridding3dsingle_homodyne(kvec,kdata,dcf,imgN,...
    a,kwidth,nk_low,niter,padN,vdim,crop)
...
if nargin<9, padN = imgN; end
if nargin<10, vdim = [1 1 1]; end
vdim = vdim/min(vdim);
if nargin<11, crop = true; end
...
if false
    ...
    ii = 1:nk_low;
    gdat_low = gr3dKB_float_mex(kvec(ii,1), kvec(ii,2), kvec(ii,3), ...
        kdata(:,ii), dcf(ii,1), grsize(:), ...
        kwidth/2, single(C_samp), single(vdim));    
end
...
if any(any(any(any(isnan(gdat_low))))) 
    warning('isnan(gdat_low); nulling NaNs');
    if do_debug
        save('gridding3dsingle_nan_debug_low.mat','-v7.3')
    end
    gdat_low(isnan(gdat_low)) = 0; 
end


%% 3D-IFFT
gdat_orig = ifftshift(ifftshift(ifftshift(gdat,3),2),1);
gdat = gdat_orig;
mask = gdat_orig~=0;
img_low = ifft(ifft(ifft(...
    ifftshift(ifftshift(ifftshift(gdat_low,3),2),1),[],3),[],2),[],1);
for li=1:niter
    img = ifft(ifft(ifft(gdat,[],3),[],2),[],1);
    if false
        tmp = fftshift(fftshift(fftshift(img,3),2),1);
        figure(li+10);
        imagesc_ind3d(abs(tmp),'','','','',true)
        figure(li+20);
        imagesc_ind3d(angle(tmp),'','','','',true)
    end
    img = abs(img).*img_low./abs(img_low);
    if li==niter, break; end
    gdat = fft(fft(fft(img,[],3),[],2),[],1);
    gdat(mask) = gdat_orig(mask);
end
img = fftshift(fftshift(fftshift(img,3),2),1);
...
end  % main function gridding3dsingle_homodyne.m
