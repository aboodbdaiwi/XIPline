function [spec,hz,s1d,ppm,par,h] = recon_csi_all(d,h,zf,lb,do_ecc,ws,fname,epsi,verb,...
    coco,abs_fid,freq,lcm_basis,d_ref,cor_intlv,do_ori)
%RECON_CSI_ALL  Reconstruct CSI + EPSI data (original, extensive code)
%[spec,hz,s1d,ppm,par,h]=recon_csi_all(d,h,zf,lb,do_ecc,ws,fname,epsi,verb,...
%                    coco,abs_fid,freq,lcm_basis,d_ref,cor_intlv,do_ori)
%        d  Raw p-file data [(nx*ny*nz),ns,1,nt,1,nc]
%           (ns=spectral pts;nc=coils)
...
%     verb  Verbose mode (>0) + plotting: 2=all,3=invis,4=no plot_mrsi  (2)
...
if ~isempty(fname)
    ...
    save([fname '.mat'],'-mat','-v7.3',...
        ...
        'coco','abs_fid','freq','lcm_basis','cor_intlv','do_ori',...
        'hz','s1d','nn','t','dim','bb',...
...
if ((verb>1) && (ns>1))
    if (length(size(spec))>5) || ((verb==3)&&(isempty(fname)))
        ...
        imagesc_row(bb,[0 0.95*max(bb(:))],'',size(bb,4)==1,true);
        ...
        if ~isempty(fname)
            if export_dcm>0, hh = h;
            else, hh = []; end
            plot_csi(spec,pc,[],[figstr '- CSI'],[fname '_plot_csi'],11,hh);            
        end
        
        if verb==2, plot_mrsi(spec,par); end
    end
end
...
if verb>0, fprintf('recon_csi: runtime = %.2f s\n',toc(timerVal)); end
