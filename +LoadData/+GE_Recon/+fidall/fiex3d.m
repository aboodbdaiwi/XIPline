...
if verb>1, timerVal = tic; end         % record runtime
...
n_ext = max([fi_nr ex_nr])+ex_rounds;  % extend matrix by 2*n_ext
...
%% 1) fit sub-area
if verb>0, fprintf('Polynomial smoothing %dD inside\n',dim); end
se2 = zeros(mtx_ext);
if ((fi_nr>0)&&(fi_order>0))
            ...
            for l3=ie{3}
                if (se1(l1,l2,l3)~=0)
                    ...
                    if ~isempty(msgstr)
                        ...
                        if isempty(regexpi(msgid,'singularmatrix','once')) 
                            fprintf('Warning: %s\n',msgstr);
                        end
    ...
    if verb>0, fprintf('Skipping polynomial fitting inside\n'); end
    se2 = se1;
end


%% 2) extrapolation of sensitivities
if verb>0, fprintf('Extrapolation of sensitivites\n'); end
se3 = se2;
if sum(abs(se1)>eps)==0
    if verb>0, fprintf('mask all zeroes -> skipping extrapolation\n'); end
    ex_rounds = 0;
end
if ((ex_nr>0)&&(ex_order>0)&&(ex_rounds>0))
            ...
            for l2=1:mtx_ext(2)
                for l3=1:mtx_ext(3)
                    if diff_mask(l1,l2,l3)
                        ...
                        X1 = X(isi,:);
                        W1 = W(:,isi);
                        c = (W1.*X1'*X1)\((W1.*X1')*s(isi));    % LSQ fit
                        % c = pinv(W1.*X1'*X1)*(W1.*X1')*s(isi);
                        msgstr = lastwarn;
                        if ~isempty(msgstr)
                            nwarn = nwarn+1;
    ...
    if verb>0, fprintf('Skipping polynomial extrapolation outside\n'); end
end
...
if verb>1, fprintf('fiex3d: runtime = %.2f [s]\n',toc(timerVal)); end
