...
if do_rot_file && hub(1)~=3
    error('do_rot_file && hub(1)(=%d)~=3',hub(1));
end
...
if nintlv*hub(1)>RFS_MAX_NSCANS
    warning('nintlv(=%d)>%d: exceeding fidall limit',nintlv*hub(1),RFS_MAX_NSCANS);
end
...
switch hub(1)
    ...
    case 3
        if do_rot_file
            ...
            theta = [0,90,90];
            ksp = cat(2,...
                fidall_vap_rotation(ksp,phi(1),theta(1)),...
                fidall_vap_rotation(ksp,phi(2),theta(2)),...
                fidall_vap_rotation(ksp,phi(3),theta(3)));
        else
...
switch order(1)
    ...
    case 1         % random trajectory order
        if ~do_rot_file
            ind_order = randperm(nintlv);
        else
            ind_order = randperm(nintlv3);
            ind_order = [ind_order, ind_order+nintlv3, ind_order+2*nintlv3];
        end
        ksp = ksp(:,ind_order,:);
        orderstr = '_R';
    case 2         % mix every order(2)-th spiral
        ...
        ind_order = [];
        for ll=1:order(2)
            ind_order = [ind_order,ll:order(2):nintlv];
        end
        if length(ind_order)~=nintlv
            error('length(ind_order)(=%d)~=nintlv(=%d)',...
                length(ind_order),nintlv); 
        end
...
%% bonus spec
if bonus_spec>0
    nintlv = nintlv+bonus_spec;
    ksp_zeros = zeros(nn(2),bonus_spec,3);
    ksp = [ksp, ksp_zeros];
...
if calc_dcf
    ...
    tmp_k = k(:,1:((nintlv-bonus_spec)*nk),:); % don't include spec
...
if (islogical(fname) || isnumeric(fname))
    if fname
        ...
        fname = sprintf('floret_%s_fov%g_mtx%g_arms%g_hub%d_%g_intlv%g_bonus%g_kdt%g_gmax%g_smax%g_dur%g%s%s', ...
            nuc,round(fov*1d3),mtx,arms,hub(1),round(hub(2)*10)/10,nintlv,bonus_spec,kdt*1d6,...
...
if ~isempty(fname)
    ...
    if do_rot_file
        if hub(1)==3
            out = write_ak_wav([fname '.wav'],grad(:,1:nintlv3,:),1/kdt,fov_mns,desc,acq_pts,...
                [],[],gdt2*1d6);
            write_fdl([fname '_phi.fdl'],  repelem(phi,nintlv3),  'phi');
            write_fdl([fname '_theta.fdl'],repelem(theta,nintlv3),'theta');
        else
            warning('do_rot_file only for hub(1)=3; not saving grads');
        end
    else
