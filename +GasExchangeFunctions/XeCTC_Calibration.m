function [GasExResults, CalResults] = XeCTC_Calibration(DataFile)
%XeCTC_Calibration - Analysis of Xe CTC calibration scan
...
if exist('DataFile','var') == 0
  [DataFile, DataPath] = uigetfile('*.data;*.DATA', 'Select a .data file');
  DataFile = fullfile(DataPath, DataFile);
end
theFID = LoadData.loadLISTDATA(DataFile);
...
gasfitObj = GasExchangeFunctions.GasExchange_Spectro.NMR_TimeFit(gasData,t,1e-4,-84,30,0,0,10000);
...
if Voigt == 1
    ...
    disfitObj = GasExchangeFunctions.GasExchange_Spectro.NMR_TimeFit_v(disData1_avg,t,[1 1 1],[0 -720  -7700],[250 200 30],[0 200 0],[0 0 0],0,length(t)); % first widths lorenzian, 2nd are gauss
else
    fprintf('Using Lorentzian Lineshape to fit Barrier\n');
    disfitObj = GasExchangeFunctions.GasExchange_Spectro.NMR_TimeFit(disData1_avg,t,[1 1 1],[0 -720  -7700],[240 240 40],[0 0 0],0,length(t));
end
