...
function LBVDP_Report(Ventilation, Proton, MainInput)
    ...
    % PDF export has no direct macOS/Linux equivalent (no PowerPoint/LibreOffice
    % COM automation available), so it's skipped there; the pptx report above is
    % still saved on every platform. The "Batch Analysis Preview" copy step below
    % already guards on exist(PDFoutputPath,'file'), so it no-ops automatically.
    if ispc
