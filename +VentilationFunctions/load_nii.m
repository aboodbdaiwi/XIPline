...
function nii = load_nii(filename, img_idx, dim5_idx, dim6_idx, dim7_idx, ...
   ...
   %
   [nii.hdr,nii.filetype,nii.fileprefix,nii.machine] = Functions.load_nii_hdr(filename);
...
%   nii.ext = load_nii_ext(filename);
   ...
   %
   [nii.img,nii.hdr] = Functions.load_nii_img(nii.hdr,nii.filetype,nii.fileprefix, ...
		nii.machine,img_idx,dim5_idx,dim6_idx,dim7_idx,old_RGB);

   %  Perform some of sform/qform transform
   %
   nii = Functions.xform_nii(nii, tolerance, preferredForm);
