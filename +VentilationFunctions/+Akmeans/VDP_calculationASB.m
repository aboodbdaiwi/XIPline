...
for slice = 1:size(maskarray,3)
    ...
    axes(ax1);
    [~,~] = Global.imoverlay( ...
        squeeze(abs(HImage(:,:,slice))), ...
        squeeze(maskboundaries), ...
        [1,100], ...
        [0,max(HImage(:))], ...
        cus_colormap, ...
        1, ...
        ax1);
    colormap(ax1,cus_colormap)
    drawnow limitrate;
...
if numel(tiff_info) ~= size(maskarray,3)
    warning('Expected %d slices, but saved TIFF contains %d slices.', ...
        size(maskarray,3), numel(tiff_info));
end
Mask_Proton_boundaries = uint8(zeros( ...
    tiff_info(1).Height, ...
    tiff_info(1).Width, ...
    3, ...
    numel(tiff_info)));
for ii = 1:numel(tiff_info)
    Mask_Proton_boundaries(:,:,:,ii) = imread(tiffFile,ii);
end
