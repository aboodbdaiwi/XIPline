function  [cleaned_mask,cluster_stack,proton_stack,...
    truncated_vessel_mask,transformed_data,low_intn_percent,ventilation_mask,lungsmask_corrected] = ventilation_defect_by_kmeans_oakes(temp_handles)
%%
...
if strcmp(temp_handles.view,'coronal')
    ...
    cluster_stack = ventCluster(he,lungsmask_corrected,num_clusters,cluster_option);
else
    cluster_stack=zeros(nRows,nCols,nSlices);
    for n_sl=1:nSlices
        cluster_stack(:,:,n_sl) = ventCluster(he(:,:,n_sl),lungsmask_corrected(:,:,n_sl),num_clusters,cluster_option);
