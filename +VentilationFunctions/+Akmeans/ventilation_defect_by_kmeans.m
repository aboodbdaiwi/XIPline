function  [cleaned_mask,cluster_stack,proton_stack,...
    truncated_vessel_mask,transformed_data,low_intn_percent,ventilation_mask,lungsmask_corrected] = ventilation_defect_by_kmeans(temp_handles)
%%
...
%     maxhe = max(slice_he(:));
%     minhe = min(slice_he(:));
%     he_flat(:,:,n_sl) = (slice_he-minhe)*256/(maxhe-minhe);
% end
...
switch temp_handles.view
    case 'axial'
        diaphragm_sl = VentilationFunctions.Akmeans.locate_diaphragm_slice(lungsmask);
        vasculature_slices = contoured_slices(2): (diaphragm_sl+1);
        diaphragm_slices = diaphragm_sl:contoured_slices(end);
    case 'coronal'
        
        vasculature_slices = contoured_slices(3:(end-2));
        diaphragm_slices = 0;
        lungsmask = VentilationFunctions.Akmeans.fill_lungsmask(lungsmask,temp_handles.view);
    otherwise
...
if isfield(temp_handles,'registered_proton')
    transformed_data=VentilationFunctions.Akmeans.hist_transformation(temp_handles.registered_proton.proton_reg,temp_handles.hist_mu);
else
    transformed_data=VentilationFunctions.Akmeans.hist_transformation(temp_handles.proton,temp_handles.hist_mu);
end
...
if strcmp(temp_handles.view,'coronal')
    outter_boundary = VentilationFunctions.Akmeans.split_lung_boundaries_coronal(temp_handles.lungsmask,1:nSlices);
else
    outter_boundary = VentilationFunctions.Akmeans.split_lung_boundaries(temp_handles,1:nSlices);
end
...
if ~isfield(temp_handles,'vessel_mask')
    for sl = vasculature_slices
        ...
        try
            [~,~,~,filtered_at_sigmas]= Segmentation.frangi_filterv2.FrangiFilter2D(frangi_proton(:,:,sl),options);
        catch
            continue;
        end
...
cutoff_low = 0.035;
% if isfield(temp_handles,'SliceThickness') && temp_handles.SliceThickness<=10
%     cutoff_low = 0.03;
% end
if low_intn_percent<cutoff_low % the relative number of low intensity pixels decides the size of the 1st cluster of Kmeans
    cluster_option=1;
    num_clusters=5;
elseif low_intn_percent<.1 && low_intn_percent>=cutoff_low
    cluster_option=3;
...
VentLevels = 5;
vent_stack = VentilationFunctions.Akmeans.kmeans_clustering(vent_mask.*he_nlz,VentLevels);

cl4 = zeros(nRows,nCols,nSlices);
cl4(vent_stack==4)=1;
cl4_stack = VentilationFunctions.Akmeans.kmeans_clustering(cl4.*he_nlz,VentLevels);


cl2 = zeros(nRows,nCols,nSlices);
cl2(vent_stack==2)=1;

cl2_stack = VentilationFunctions.Akmeans.kmeans_clustering(cl2.*he_nlz,VentLevels);
